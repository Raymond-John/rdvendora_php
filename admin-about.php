            ...
            <table class="data-table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><?php if($member['avatar'] && file_exists($member['avatar'])): ?><img src="<?= htmlspecialchars($member['avatar']) ?>" class="avatar-preview"><?php else: ?>—<?php endif; ?></td>
...
<script>
    ...
    function editTeamMember(member) {
        ...
        if (member.avatar && member.avatar !== '') {
            previewDiv.innerHTML = `<img src="${member.avatar}" style="width:60px; height:60px; border-radius:50%; object-fit:cover; border:2px solid var(--border-primary);"> <span style="font-size:12px;">Current image</span>`;
        } else {
