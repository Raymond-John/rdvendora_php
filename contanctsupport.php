...
// Ensure store exists (optional for contact page)
...
// Subscription check – don't block, just show a warning
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'send_contact') {
    ...
    if (empty($name) || empty($email) || empty($subject) || empty($message)) {
        ...
        $to = 'support@RD Vendora.com'; // Change to your support email
        ...
        if ($mailSent) {
            $contact_message = 'Your message has been sent. We will get back to you soon.';
            $contact_type = 'success';
        } else {
            $contact_message = 'Failed to send message. Please try again later.';
            $contact_type = 'error';
        }
    }
}
...
<head>
    ...
    <title>Contact Support - RD Vendora</title>
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700;800&display=swap" rel="stylesheet">
    <style>
        /* ============================================================
           EXACT DASHBOARD STYLES (copied from your dashboard.php)
           ============================================================ */
        :root {
            --bg-primary: #f8f9fb;
            --bg-secondary: #ffffff;
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .theme-toggle .icon-sun, .theme-toggle .icon-moon { transition: all var(--transition-base); }
        ...
        .alert-error { background: var(--error-light); color: var(--error-dark); border: 1px solid #fecaca; }
...
<body>
    ...
    <aside class="sidebar" id="sidebar">
        ...
        <nav class="sidebar-nav">
            ...
            <a href="contact" class="sidebar-link active">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg>
                <span class="sidebar-link-text">Contact Support</span>
            </a>
    ...
    <div class="main-content">
        ...
        <div class="page-content">
            <div class="page-header">
                ...
                <p class="page-subtitle">Get help with your store, report issues, or ask questions.</p>
            </div>

            <?php if (!$hasSubscription): ?>
                <div class="alert alert-warning">
                    ⚠️ Your store is currently suspended because there is no active subscription. You can still contact support, but store functionality is limited. <a href="subscription" style="color: var(--primary); font-weight: 600;">Reactivate now →</a>
                </div>
            <?php endif; ?>

            <?php if ($contact_message): ?>
                <div class="alert alert-<?= $contact_type ?>"><?= htmlspecialchars($contact_message) ?></div>
            <?php endif; ?>
                ...
                <div class="contact-info">
                    ...
                    <div class="contact-detail">
                        ...
                        <div>
                            ...
                            <div style="color: var(--text-secondary);">+234 123 456 7890</div>
                        </div>
                    </div>
                    <div class="contact-detail">
                        <div class="contact-detail-icon">
                            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>
                        </div>
                        <div>
                            <div style="font-weight: 600;">Email</div>
                            <div style="color: var(--text-secondary);">support@RD Vendora.com</div>
                        </div>
                    </div>
                    <div class="contact-detail">
                        <div class="contact-detail-icon">
                            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg>
                        </div>
                        <div>
                            <div style="font-weight: 600;">Address</div>
                            <div style="color: var(--text-secondary);">Lagos, Nigeria</div>
                        </div>
                    </div>
                    <div class="contact-detail">
                        <div class="contact-detail-icon">
                            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M12 8v4l3 3M12 2a10 10 0 1 0 0 20 10 10 0 0 0 0-20z"/></svg>
                        </div>
                        <div>
                            <div style="font-weight: 600;">Business Hours</div>
                            <div style="color: var(--text-secondary);">Mon–Fri, 9am – 6pm WAT</div>
                        </div>
                    </div>
                </div>
                ...
                <div class="contact-form-card">
                    ...
                    <form method="POST" action="">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="name" class="form-input" required value="<?= htmlspecialchars($_SESSION['fullname']) ?>">
                        </div>
                        <div class="form-group">
                            <label class="form-label">Email Address</label>
                            <input type="email" name="email" class="form-input" required value="<?= htmlspecialchars($_SESSION['email'] ?? '') ?>">
                        </div>
                        <div class="form-group">
                            <label class="form-label">Subject</label>
                            <input type="text" name="subject" class="form-input" required placeholder="e.g., Billing question, Technical issue">
                        </div>
                        <div class="form-group">
                            <label class="form-label">Message</label>
                            <textarea name="message" class="form-textarea" required placeholder="Please describe your issue or question in detail..."></textarea>
                        </div>
    ...
    <script>
        // ============================================================
        // EXACT DASHBOARD JS (sidebar, theme, notifications, etc.)
        // ============================================================
        // Notifications (mock)
        let notifications = [
            { id: 1, title: 'Welcome to RD Vendora', text: 'Thank you for choosing RD Vendora!', time: '2 days ago', unread: false },
            { id: 2, title: 'New Feature', text: 'Subscription plans now available.', time: '1 week ago', unread: false }
        ];
        ...
        function markNotificationRead(id) { const n = notifications.find(n => n.id === id); if (n) n.unread = false; renderNotifications(); }
        ...
        if (themeToggle) {
            themeToggle.addEventListener('click', () => { const newTheme = htmlEl.getAttribute('data-theme') === 'light' ? 'dark' : 'light'; htmlEl.setAttribute('data-theme', newTheme); localStorage.setItem('RD Vendora-theme', newTheme); });
        }
        ...
        function toggleMobileSidebar() { sidebar.classList.toggle('mobile-open'); overlay.classList.toggle('active'); document.body.style.overflow = sidebar.classList.contains('mobile-open') ? 'hidden' : ''; }
        if (sidebarToggle) {
            sidebarToggle.addEventListener('click', () => { if (window.innerWidth <= 768) toggleMobileSidebar(); else sidebar.classList.toggle('collapsed'); });
        }
        ...
        document.addEventListener('click', (e) => {
            ...
            if (notifDropdown && !notifDropdown.contains(e.target)) notifDropdown.classList.remove('open');
            else if (notifDropdown && e.target.closest('.dropdown-trigger')) notifDropdown.classList.toggle('open');
        });
    </script>
</body>
</html>