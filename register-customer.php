...
// If already logged in, verify user exists
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['register'])) {
    ...
    if (empty($full_name) || empty($email) || empty($password) || empty($confirm)) {
        ...
        if ($stmt->num_rows > 0) {
            ...
            $stmt = $conn->prepare("INSERT INTO users (full_name, email, password_hash, phone, role) VALUES (?, ?, ?, ?, 'customer')");
...
<head>
    ...
    <title>Register – RD Vendora</title>
...
<div class="auth-container">
    ...
    <div class="auth-sub">Join us and start shopping today.</div>
    ...
    <form method="POST" action="">
        <div class="form-group">
            ...
            <input type="text" id="full_name" name="full_name" placeholder="John Doe" value="<?= htmlspecialchars($_POST['full_name'] ?? '') ?>" required>
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
            ...
            <input type="tel" id="phone" name="phone" placeholder="+234 800 000 0000" value="<?= htmlspecialchars($_POST['phone'] ?? '') ?>">
        </div>
        <div class="form-group">
        ...
        <button type="submit" name="register" class="btn-primary"><i class="fas fa-user-plus"></i> Sign Up</button>
    </form>

    <div style="text-align:center; margin-top:1.5rem; font-size:0.9rem; color:#6b7280;">
        Already have an account? <a href="login-customer.php" style="color:#e63a2e; font-weight:600; text-decoration:none;">Sign In</a>
    </div>
</div>
</body>
</html>